function ck_vis_out()
        erdata = load('ck_er_vis.mat');
        iddata = load('ck_id_vis.mat');
        aim = iddata;
        figure;
        ...
        for i = 1:8
                axes(hs(3*(i-1)+1));
                showMeanImage(aim.imgdata,i+8);
                if(i==1)
                        title('input');
                end
                axes(hs(3*(i-1)+2));
                showMeanImage(aim.conv1,i+8);
                if(i==1)
                        title('conv1');
                end
                axes(hs(3*(i-1)+3));
                showMeanImage(aim.conv2,i+8);
                if(i==1)
                        title('conv2');
                end
        end  
        ...
        for i = 1:8
                axes(hs(3*(i-1)+1));
                showMeanImage(aim.imgdata,i+16);
                if(i==1)
                        title('input');
                end
                axes(hs(3*(i-1)+2));
                showMeanImage(aim.conv1,i+16);
                if(i==1)
                        title('conv1');
                end
                axes(hs(3*(i-1)+3));
                showMeanImage(aim.conv2,i+16);
                if(i==1)
                        title('conv2');
                end
        end  
